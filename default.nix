...
in {
  zellij = craneLib.buildPackage {
    src = ./.;
    nativeBuildInputs = [pkgs.pkg-config rustPkgs.libiconv rustPkgs.openssl] ++ pkgs.lib.optionals pkgs.stdenv.isDarwin (with pkgs.darwin.apple_sdk.frameworks; [DiskArbitration Foundation]);
